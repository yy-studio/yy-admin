    ...
    <a-form class="smart-query-form">
        <a-row class="smart-query-form-row">
            <a-form-item label="关键字查询" class="smart-query-form-item">
                <a-input style="width: 150px" v-model:value="queryForm.keywords" placeholder="昵称/手机号" />
            </a-form-item>
        ...
        <a-table size="small" :dataSource="tableData" :columns="columns" rowKey="id" bordered :loading="tableLoading"
            ...
            <template #bodyCell="{ text, record, column }">
                ...
                <template v-if="column.dataIndex === 'status'">
                    {{ text == 1 ? '正常' : text == 0 ? '未激活' : '注销' }}
                </template>
                <template v-if="column.dataIndex === 'action'">
                    <div class="smart-table-operate">
                        <a-button @click="showForm(record)" type="link">编辑</a-button>
                        <a-button @click="onDelete(record)" danger type="link">注销</a-button>
                    </div>
                </template>
            </template>
        </a-table>
...
async function queryData() {
    ...
    try {
        let queryResult = await userApi.queryPage(queryForm);
